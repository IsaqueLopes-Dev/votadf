...
function AuthPageContent() {
  ...
  const safeNextPath = requestedNext && requestedNext.startsWith('/') ? requestedNext : '/usuarios';
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth?next=${encodeURIComponent(safeNextPath)}`,
          queryParams: {
  ...
  if (checkingAuth) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-blue-700 via-blue-600 to-blue-50 flex items-center justify-center p-4" style={{ fontFamily: 'var(--font-poppins), sans-serif' }}>
